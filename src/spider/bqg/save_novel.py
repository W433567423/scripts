...
def save_novel_list(novel_list: list):
    with FrameProgress(
        ...
        "[cyan]⏳",
        TimeRemainingColumn(),
    ) as progress, ThreadPoolExecutor(max_workers=2) as executor:
        task = progress.add_task("正在保存小说至本地", total=len(novel_list))
        task_list = []
        for novel in novel_list:
            chapters_list = get_chapters_list_from_db(novel["book_id"])
            task_list.append(executor.submit(get_chapters_list, novel, chapters_list,progress))
        for task in as_completed(task_list):
            progress.update(task, advance=1)
        wait(task_list, return_when=ALL_COMPLETED)
        progress.update(task, advance=len(novel_list))

        # for novel in novel_list:
        #     get_chapter_content(novel, chapters_list,progress)
        #     progress.update(task, advance=1)
        #     pass

# 获取某章节内容(用于submit)
def get_chapter_content_thread(novel, chapter_list,progress):
    book_content=f"""
小说名: {novel["book_name"]}
...
chapter["chapter_name"]
    ...
    for content in contents:
        ...
        #     pass
            ...
            chapter_content=chapter_content+ str(content).replace("\xa0"," ")+'\n'
